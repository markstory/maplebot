...
Weather.prototype.help = function () {
	return [
		'Weather',
		'--------',
		'!weather <city>: Get the current conditions for a city.',
		'!forecast <city>: Get the 5 day forecast for a city.'
	].join('\n');
};
...
function query(location, callback) {
	var options = {
		host: 'www.google.com',
		path: '/ig/api?weather=' + encodeURIComponent(location.body.trim())
	};
